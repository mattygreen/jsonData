...
fetch('https://raw.githubusercontent.com/mattygreen/jsonData/main/data26thNov.json')
    ...
    .then(data => {
       ...
       for (let i = 1; i < data.length; i++) {
        ...
        const filteredNewData = newData.filter(item => item.theme === 'Police' || item.theme === 'labels')
...
function createTimeLineStructure(data) {
    ...
    const items = document.querySelectorAll(".timelinebox");items.forEach(item => {
        item.style.borderLeft = "20px solid darkolivegreen";})
...
const downArrow = document.getElementById('downArrowPolice');
