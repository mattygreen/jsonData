...
fetch('https://raw.githubusercontent.com/mattygreen/jsonData/main/data26thNov.json')
    ...
    .then(data => {
       ...
       for (let i = 1; i < data.length; i++) {
        ...
        const filteredNewData = newData.filter(item => item.theme === 'Racial Discrimination' || item.theme === 'labels')
...
function createTimeLineStructure(data) {
    ...
    data.forEach(item => {
        ...
        items.forEach(item => {
          item.style.borderLeft = "20px solid green";
        ...
        if (item.hasOwnProperty('audiofile2')) {
            const audiotitle2 = document.createElement('h4')
                audiotitle2.innerText = `${item.audiocontent2}`;
                ...
                modalBody.appendChild(audioTag2);
                audioTag2.appendChild(audiotagsource2);
                console.log(audiotagsource2); 
        }
...
const downArrow = document.getElementById('downArrowRacialDiscrimination');
